					...
					<div class="skills-vault-table">
						...
						<table class="table skills-vault-table" id="skills-vault-table" width="100%">
							...
							<tbody>
								...
								if (empty($skills) || sizeof($skills) <= 0) {
								?>
									<tr>
									...
									foreach ($skills as $skill) {
										...
										if (sizeof($skillTypes) > 0) {
											$sizeof = 0;
											foreach ($skillTypes as $skillType) {
												if (++$sizeof > 1 && $sizeof <= sizeof($skillTypes)) {
													$skillTypeString .= ', ';
												}
										...
										if (sizeof($performanceLevels) > 0) {
											...
											foreach ($performanceLevels as $performanceLevel) {
												if (++$sizeof > 1 && $sizeof <= sizeof($performanceLevels)) {
													$performanceLevelString .= ', ';
												}
									...
									?>
										<tr>
											...
											<td class="<?= sizeof($skillExamples) > 0 ? 'multiple-actions' : '' ?>">
												...
												if (!empty($videoCode)) {
												?>
													<a class="BTN action-button" data-title="<?= $name ?>" data-url="<?= $url ?>" data-button="Full Breakdown" data-video="<?= htmlspecialchars($videoCode) ?>" data-side="<?= htmlspecialchars($breakdownDetails) ?>">
