...
<section class="dashboardbenefits" style="min-height: 80vh; padding: 0 0 2rem 0;">
	...
	<div class="challenges-wrapper">
		...
		<div class="bootstrap-styles challenges">
			...
			?>
				<a href="<?php if (get_field('available') && (!$isLatestChallenge || current_user_can('memberpress_authorized'))) { ?><? the_permalink();
																																	} else { ?>#latestChallengeModal<?php } ?>" class="card shadow challenge filter <?php echo $class; ?>" <?php
																																																																if ($isLatestChallenge && !current_user_can('memberpress_authorized')) {
																																																																?> data-toggle="modal" data-target="latestChallengeModal" <?php
																																																																}
																						?> data-category="<?= $termsString ?>" data-sort="value" data-search="<?php the_title(); ?>\n<?php echo get_field('description_short'); ?>">
					<?php
					$isMembersOnlyChallenge = false;
					if (
						!(!get_field('available') && (!$isLatestChallenge || current_user_can('memberpress_authorized'))) &&
					...
					?>
						...
						if (!get_field('available') && (!$isLatestChallenge || current_user_can('memberpress_authorized'))) {
							...
							<div class="card-body">
								<h4><? the_title(); ?></h4>
								<p class="filterDesc card-text"><?php echo get_field('description_short'); ?></p>
							</div>
				</a>
			<?php
			endwhile;
